...
import csv
from utils_2 import stock_state
from utils_2.dataLoader import *
import tqdm
import re
import datetime
import os
import torch
from utils_2.Helper_Functions import *
...
class agent(): # noqa
    def __init__(self, num_node_features=6, action_shape=19, lr=0.001, gamma=0.9, train_episodes=TRAIN_EPISODES):
        # weights = torch.empty(state_shape, action_shape)
        # torch.nn.init.uniform(weights)
        self.actions_shape = action_shape
        self.model = GCN(num_node_features=num_node_features)
        self.gamma = gamma
        ...
        self.target_model = GCN(num_node_features=num_node_features)
    ...
    def train(self, replay_memory, curr_episode, batch_size=64):  # model=Q, target_model = Q_tilda
        ...
        current_qs_list = self.model(create_graph_batch(current_states, DEVICE))[:batch_size*3, :].reshape(-1, 6)  # noqa
        future_qs_list = self.target_model(create_graph_batch(new_current_states, DEVICE))[:batch_size*3, :].reshape(-1, 6)  # noqa
    ...
    def decide_action(self, observation, epsilon):
        ...
        if random_number <= epsilon:
            ...
            predicted = self.model(observation).reshape(-1, )[:NUM_OF_ACTIONS]
...
def main():
    ...
    obs = env.observation()
    env_agent = agent(num_node_features=obs.num_node_features, action_shape=NUM_OF_ACTIONS, lr=lr, gamma=gamma, train_episodes=TRAIN_EPISODES)  # noqa - state_space_shape = 3x2 + 1(ration dis) , action_space_shape=3*3*2 + 1

    replay_memory = deque(maxlen=MAX_RB_LENGTH)  # noqa - state
    ...
    for episode in tqdm.tqdm(range(START_EP, TRAIN_EPISODES)):
        # environment
        env = stock_state.env(init_UPRO_val=UPRO_values[0], init_TMF_val=TMF_values[0], num_of_actions=NUM_OF_ACTIONS-1)
        observation  = env.observation()  # noqa
        first_reward = env.reward()  # noqa
        last_reward  = first_reward  # noqa
        mean_loss = []
        for i, (upro, tmf) in enumerate(zip(UPRO_values[1:], TMF_values[1:])): # noqa
            ...
            observation.x[0, 0] = upro / 10
            observation.x[1, 3] = tmf
            # 2. Explore using the Epsilon Greedy Exploration Strategy
            action = env_agent.decide_action(observation=observation.to(DEVICE), epsilon=epsilon)
            new_observation, new_reward = env.step(action)
            replay_memory.append([observation.to(DEVICE), action, new_reward - last_reward, new_observation.to(DEVICE)])
